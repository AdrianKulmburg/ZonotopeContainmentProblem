...
for i_m2 = 1:size_m2_range
    ...
    full_data = ver_point_data{i_m2}{5};
    s = size(full_data,2);
    smaller = full_data(1:s/2);
    equal = full_data(s/2+1:end);
...
%ylim([0 60])
lgd = legend([u1 u2 u3], {'\verb|ZC|$^{\verb|O|}$', '\verb|ZC|$^{\verb|m|_1}$', '\verb|ST|'}, 'Interpreter', 'latex', 'Location', 'northwest', 'Orientation', 'horizontal');
lgd.FontSize = 13;
...
ylabel('Runtime, in [s]', 'Interpreter', 'latex', 'FontSize', 13)
ylim([0 12])
lgd = legend([d1 d2 d3], {'\verb|ZC|$^{\verb|O|}$', '\verb|ZC|$^{\verb|m|_1}$', '\verb|ST|'}, 'Interpreter', 'latex', 'Location', 'northwest', 'Orientation', 'horizontal');
lgd.FontSize = 13;
...
% annotation('textbox', [0.898, 0.465, 0.098, 0.12], 'EdgeColor', 'black')
