...
for i_m2 = 1:size_m2_range
    ...
    % Next, plotting the results from ZC^n
    
    worst_case = poly_point_data{i_m2}{3};
    ...
    mean = poly_point_data{i_m2}{2};
    all_points = poly_point_data{i_m2}{5};
    sigma = std(all_points);
...
subplot(2, 1, 1);
hold off
title('Worst-case runtime of \verb|ZC|$^{\verb|O|}$, \verb|ZC|$^{\verb|n|}$, and \verb|ST| for $n = 5, m_1 = 10$.', 'Interpreter', 'latex', 'FontSize', 13)
xlabel('$m_2$', 'Interpreter', 'latex', 'FontSize', 13)
ylabel('Runtime, in [s]', 'Interpreter', 'latex', 'FontSize', 13)
ylims_max = ylim;
lgd = legend([u1 u2 u3], {'\verb|ZC|$^{\verb|O|}$', '\verb|ZC|$^{\verb|n|}$', '\verb|ST|'}, 'Interpreter', 'latex', 'Location', 'northwest');
lgd.FontSize = 9;

subplot(2, 1, 2);
hold off
title('Mean runtime of \verb|ZC|$^{\verb|O|}$, \verb|ZC|$^{\verb|n|}$, and \verb|ST| for $n = 5, m_1 = 10$.', 'Interpreter', 'latex', 'FontSize', 13)
xlabel('$m_2$', 'Interpreter', 'latex', 'FontSize', 13)
ylabel('Runtime, in [s]', 'Interpreter', 'latex', 'FontSize', 13)
ylim(ylims_max);
